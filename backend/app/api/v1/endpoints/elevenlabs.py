...
async def get_transcript(data: ElevenLabsSchema, token=Depends(verify_bearer_token)):
    ...
    for i in range(0, len(filtered_transcript) - 1, 2):
        if (
            filtered_transcript[i]["source"] == "ai"
            and filtered_transcript[i + 1]["source"] == "user"
        ):
    ...
    product_description_str = (
        product.get("description") if product.get("description") else ""
    )
    ...
    try:
        ...
        if cleaned_complete_eval.startswith(
            "```json"
        ):  # Be more specific with json marker
        ...
        if isinstance(complete_eval_json, dict):
            ...
            if "complete_evaluation" in complete_eval_json and isinstance(
                complete_eval_json["complete_evaluation"], dict
            ):
                complete_evaluation_to_save = complete_eval_json[
                    "complete_evaluation"
                ]  # Save the nested evaluation object
            elif "complete_evaluation" in complete_eval_json:
                complete_evaluation_to_save = complete_eval_json[
                    "complete_evaluation"
                ]  # Save the value even if not a dict (e.g., string fallback from LLM)

            if "complete_rating" in complete_eval_json and isinstance(
                complete_eval_json["complete_rating"], dict
            ):
                complete_rating_to_save = complete_eval_json[
                    "complete_rating"
    ...
    if final_additional_criteria_config:
        ...
        additional_criteria_evaluation = (
            "No additional criteria selected for this test configuration."
        )
    ...
    for idx, exchange in enumerate(transcript_text):
        ...
        try:
            ...
            parsed_evaluation_text = parsed_json.get(
                "evaluation", cleaned_indiv_eval
            )  # Fallback to raw if "evaluation" key is missing
            parsed_rating = parsed_json.get(
                "rating"
            )  # Can be None if "rating" key is missing
    ...
    for i in range(3, len(transcript_text), 4):
        mid_eval = evaluate_mid_conversation(
            transcript_text[: i + 1], combined_product_context, current_persona
        )
    ...
    evaluation_data = {
        ...
        "test_configuration_id": (
            ObjectId(test_config_id_str) if test_config_id_str else None
        ),  # Convert to ObjectId
    }
    ...
    try:
        ...
        category = category_collection.find_one({"_id": product["category_id"]})
        test_config = (
            test_configurations_collection.find_one(
                {"_id": ObjectId(test_config_id_str)}
            )
