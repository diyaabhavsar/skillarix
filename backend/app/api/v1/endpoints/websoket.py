...
@router.websocket("/ws/chat-practice")
async def websocket_endpoint(websocket: WebSocket, token: str = Query(None)):
    await websocket.accept()
    ...
    try:
        while True:
            ...
            if data.get("type") != "generate_question":
                await websocket.send_json({"error": "Unsupported message type"})
                continue

            product_id_str = data.get("product_id")
            test_config_id_str = data.get("test_configuration_id")
            conversation_history = data.get("history", [])

            if not product_id_str:
                await websocket.send_json({"error": "product_id is required"})
                continue
            ...
            # Load persona from test configuration (if provided)
            persona = {'product_knowledge': 'peer-heard', 'product_familiarity': 'handled-briefly', 'technical_expertise': 'basic', 'key_challenges': 'quality', 'buying_objective': 'save-money', 'budget_range': 'low', 'decision_authority': 'approver', 'exhibition_objective': 'partnership'}

            combined_product_context = "This Xiaomi Fire TV comes with FantastiQLED, Smart Fire OS, and Quantum Dot display to facilitate smoother visuals. Additionally, this TV offers 4 times the details of FHD resolution, fostering maximum picture clarity. Moreover, this TV features HDR10+, DCI-P3 Colour Gamut, Reality Flow, Filmmaker Mode, 34W box speakers, Alexa Voice Seach, and Picture-in-Picture Mode. Furthermore, this TV allows fast streaming, boundless entertainment, and optimised performance. It also comes equipped with 32 GB internal storage, remote control, and connectivity ports. "
