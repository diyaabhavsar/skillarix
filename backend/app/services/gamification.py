...
async def award_xp(user_id: str, amount: int, reason: str, metadata: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[SUCCESS] Awarded {amount} XP to {user_id}. New Total: {new_total_xp}")
...
async def check_milestones(user_id: str, category: str, value: int):
    ...
    for ms in active_milestones:
        ...
        if not user_ms:
            ...
            if is_completed:
                print(f"[ACHIEVEMENT] Milestone completed (new): {ms['name']}")
            ...
            if current >= old_val: # changed > to >= to ensure sync
                ...
                if current != old_val or newly_completed:
                     ...
                     if newly_completed:
                        print(f"[ACHIEVEMENT] Milestone completed (update): {ms['name']}")
                        await award_xp(user_id, ms["reward_xp"], f"milestone_completed: {ms['name']}")
...
async def sync_user_gamification(user_id: str):
    ...
    """
    # print("Syncing gamification for user " + str(user_id).encode("ascii", "ignore").decode("ascii"))
    
    # 1. Sync Sessions Count
    sessions_count = db["conversations"].count_documents({"user_id": ObjectId(user_id), "is_deleted": {"$ne": True}})
    print(f"[STATS] Sessions Count: {sessions_count}")
    ...
    print(f"[TREND] Scores found: {scores}")
    ...
    if scores:
        ...
        if len(scores) >= MIN_SESSIONS_FOR_SCORE:
            ...
            print(f"[STAR] Calculated Average Score: {avg_score} -> {avg_percentage}% (from {len(scores)} sessions)")
            await check_milestones(user_id, "score", avg_percentage)
        else:
            print(f"[WARNING] User has {len(scores)} scores. Need {MIN_SESSIONS_FOR_SCORE} for average score milestones. Resetting to 0.")
            try:
                ...
                print(f"[ERROR] Error resetting milestones: {e}")
                import traceback
                traceback.print_exc()
    else:
        print("[WARNING] No scores found for user")
