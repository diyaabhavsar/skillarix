...
# Get the MongoDB collections for prompts and prompt logs
...
def get_active_prompts():
    # Find all prompts that are not deleted
...
def get_prompt_by_id(obj_id):
    # Find a prompt by its ObjectId and ensure it's not deleted
...
def soft_detele_prompt(prompt_id, user_id):
    # Soft delete a prompt by setting is_deleted to True
...
def get_logs(prompt_id):
    # Find all logs for a specific prompt
