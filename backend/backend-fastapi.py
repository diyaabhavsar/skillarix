...
class DatabaseOperations:
    ...
    def create_category(self, name: str, created_by: str) -> ObjectId:
        category_data = {
            ...
            "created_at": datetime.now(), # Use timezone-aware datetime
            "updated_at": datetime.now(), # Use timezone-aware datetime
            "is_deleted": False,
    ...
    def update_conversation_evaluation(self, conversation_id: ObjectId, evaluation_data: dict):
        ...
        result = self.conversations.update_one(
            {"_id": conversation_id},
            {"$set": {"evaluation_data": evaluation_data, "updated_at": datetime.now()}} # Use timezone-aware datetime
        )
        return result.matched_count > 0
...
class User:
    ...
    def login(self, email: str, password: str):
        ...
        user_dict['last_login'] = datetime.now().isoformat() # Return as ISO format string
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("backend-fastapi:app", host="0.0.0.0", port=8080, reload=True)  