  ...
  ({ sessions = [] }) => {
      ...
      (session: ConversationEvaluation): ConversationEvaluation => {
        ...
        return {
          ...
          evaluation_data: {
            ...
            complete_evaluation:
              session.evaluation_data.complete_evaluation || defaultEvaluation,
            ...
            additional_criteria_evaluation: session.evaluation_data
              .additional_criteria_evaluation || { distraction_handling: "" },
    ...
    const columns = useMemo<ShadcnColumn<ConversationEvaluation>[]>(
      () => [
        ...
        {
          key: "evaluation_data.score",
          header: "Score",
          className: "text-center",
          render: (value, row) => {
            const totalScore = row.evaluation_data.complete_rating?.total?.score || 0;
            const maxScore = row.evaluation_data.complete_rating?.total?.max || 100;
            return row.evaluation_data.is_complete ? (
        ...
        {
          key: "evaluation_data.status",
          header: "Status",
          className: "text-center",
          render: (_, row) =>
            renderStatus(row.evaluation_data.is_complete ? "Completed" : "In Progress"),
        },
        {
